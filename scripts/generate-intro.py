...
def write_pce(paths: list[Path], tiles: list[list[list[int]]], tilemap: bytes) -> None:
    # Keep PCE data in the exact same native 4bpp layout used by the known-good
    # gameplay charset. Three banks hold 256 tiles each; the fourth holds the
    # remaining 33 tiles and the 32x32 BAT.
...
def main() -> None:
    ...
    print(
        "PCE : "
        + ", ".join(str(p) for p in [args.pce_bank2_out, args.pce_bank3_out, args.pce_bank4_out, args.pce_bank5_out])
    )
