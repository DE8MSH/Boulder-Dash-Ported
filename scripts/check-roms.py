...
def check_snes() -> None:
    rom = read_rom(SNES)
    if len(rom) != 0x20000:
        fail(f"SNES ROM is {len(rom)} bytes; expected 131072 with animated intro banks")
    ...
    if rom[0x7FD7] != 0x07:
        fail(f"SNES ROM-size header is ${rom[0x7FD7]:02X}; expected $07 for 128 KiB")
    ...
    if all(b == 0xFF for b in rom[0x8000:0x10000]):
        fail("SNES intro tile bank is completely empty")
    if all(b == 0xFF for b in rom[0x10000:0x18000]):
        fail("SNES intro animation-map bank is completely empty")

    print(f"ok: SNES 128 KiB LoROM (4 banks), reset=${reset:04X}")


def check_pce() -> None:
    rom = read_rom(PCE)
    if len(rom) != 0x10000:
        fail(f"PC Engine ROM is {len(rom)} bytes; expected 65536 with 8 HuCard banks")
    ...
    print(f"ok: PC Engine 64 KiB HuCard (8 banks), reset=${reset:04X}")
